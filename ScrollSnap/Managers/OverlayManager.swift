...
class OverlayManager {
    ...
    private func showThumbnail(with image: NSImage) {
        ...
        let thumbnailWidth = image.size.width * thumbnailScaleFactor
        let thumbnailHeight = image.size.height * thumbnailScaleFactor
        let thumbnailSize = NSSize(width: min(thumbnailWidth, 350), height: min(thumbnailHeight, 500))
